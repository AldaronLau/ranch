...
impl<const MIN: i128, const MAX: i128> RangedI128<MIN, MAX> {
    ...
    ///
    /// ```rust
    /// # use ranch::{RangedI128, Error};
    /// RangedI128::<1, 2>::new(1).unwrap();
    /// RangedI128::<1, 2>::new(2).unwrap();
    /// assert_eq!(RangedI128::<1, 2>::new(0).unwrap_err(), Error::NegOverflow);
    /// assert_eq!(RangedI128::<1, 2>::new(3).unwrap_err(), Error::PosOverflow);
    /// ```
    pub const fn new(value: impl AsRepr<i128>) -> Result<Self> {
    ...
    /// RangedI128::<1, 3>::new_const::<1>();
    /// RangedI128::<1, 3>::new_const::<2>();
    /// RangedI128::<1, 3>::new_const::<3>();
    /// ```
    ///
    /// Does not compile:
    ///
    /// ```compile_fail
    /// RangedI128::<1, 3>::new_const::<0>();
    /// ```
    ///
    /// ```compile_fail
    /// RangedI128::<1, 3>::new_const::<4>();
    /// ```
    #[must_use]
    pub const fn new_const<const N: i128>() -> Self {
        const {
            if N < MIN || N > MAX {
                panic!("Out of bounds");
            }
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// assert_eq!(42, RangedI128::<1, 100>::new_const::<42>().get());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let n = RangedI128::<{ i128::MIN }, { i128::MAX }>::MAX;
    ///
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let n = RangedI128::<-128, 127>::new_const::<0b0101000>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<-128, 127>::new_const::<0b100_0000>();
    /// let b = RangedI128::<-128, 127>::new_const::<0b100_0011>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<1, 100>::new_const::<50>();
    /// let b = RangedI128::<1, 100>::new_const::<5>();
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<-100, 100>::new_const::<50>();
    /// let b = RangedI128::<-100, 100>::new_const::<5>();
    /// let c = a.saturating_add(b);
    /// let d = RangedI128::<-100, 100>::new_const::<-75>();
    ///
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedI128};
    /// let a = RangedI128::<-100, 100>::new_const::<50>();
    /// let b = RangedI128::<-100, 100>::new_const::<5>();
    /// let c = RangedI128::<-100, 100>::new_const::<-75>();
    ///
    ...
    /// let b = RangedI128::<-100, 100>::new_const::<5>();
    /// let c = RangedI128::<-100, 100>::new_const::<-75>();
    /// let d = RangedI128::<-100, 100>::new_const::<2>();
    ///
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedI128, Quotient};
    /// let a = RangedI128::<-100, 10>::new_const::<-50>();
    /// let b = RangedI128::<-10, 100>::new_const::<50>();
    ///
    /// assert_eq!(
    ///     a.checked_div(2),
    ///     Ok(Quotient::Number(RangedI128::new_const::<-25>())),
    /// );
    ...
    /// assert_eq!(
    ///     a.saturating_div(2),
    ///     Quotient::Number(RangedI128::new_const::<-25>()),
    /// );
    /// assert_eq!(a.saturating_div(0), Quotient::Nan);
    /// assert_eq!(
    ///     a.saturating_div(-1),
    ///     Quotient::Number(RangedI128::new_const::<10>()),
    /// );
    /// assert_eq!(
    ///     b.saturating_div(-2),
    ///     Quotient::Number(RangedI128::new_const::<-10>()),
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// assert!(!RangedI128::<-100, 100>::new_const::<10>().is_negative());
    /// assert!(RangedI128::<-100, 100>::new_const::<-10>().is_negative());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// assert!(RangedI128::<-100, 100>::new_const::<10>().is_positive());
    /// assert!(!RangedI128::<-100, 100>::new_const::<-10>().is_positive());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<-8, 8>::new_const::<0>();
    /// let b = RangedI128::<-8, 8>::new_const::<2>();
    /// let c = RangedI128::<-8, 8>::new_const::<4>();
    /// let d = RangedI128::<-8, 8>::new_const::<-1>();
    /// let e = RangedI128::<-8, 8>::new_const::<-7>();
    /// let f = RangedI128::<-8, 8>::new_const::<-3>();
    /// let g = RangedI128::<-8, 8>::new_const::<3>();
    /// let h = RangedI128::<-8, 8>::new_const::<7>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<1, 3>::new_const::<1>();
    /// let b = RangedI128::<-1, 3>::new_const::<2>();
    /// let output: RangedI128::<0, 6> = a.add(b);
    ///
    ...
    /// let a = RangedI128::<1, 3>::new_const::<1>();
    /// let b = RangedI128::<-1, 3>::new_const::<2>();
    /// let output: RangedI128::<1, 6> = a.add(b);
    ///
    /// assert_eq!(output.get(), 3);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<2, 5>::new_const::<3>();
    /// let b = RangedI128::<-1, 3>::new_const::<1>();
    /// let output: RangedI128::<-1, 6> = a.sub(b);
    ///
    ...
    /// let a = RangedI128::<2, 5>::new_const::<3>();
    /// let b = RangedI128::<-1, 3>::new_const::<1>();
    /// let output: RangedI128::<0, 6> = a.sub(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// ```rust
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<-2, 3>::new_const::<1>();
    /// let b = RangedI128::<0, 3>::new_const::<2>();
    /// let output: RangedI128::<-6, 9> = a.mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    ///
    ...
    /// let a = RangedI128::<-2, 3>::new_const::<1>();
    /// let b = RangedI128::<0, 3>::new_const::<2>();
    /// let output: RangedI128::<0, 9> = a.mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<2, 5>::new_const::<3>();
    /// let b = RangedI128::<1, 2>::new_const::<2>();
    /// let output: RangedI128::<1, 2> = a.div(b);
    ///
    ...
    /// # use ranch::RangedI128;
    /// let a = RangedI128::<2, 5>::new_const::<3>();
    /// let b = RangedI128::<1, 2>::new_const::<1>();
    /// let output: RangedI128::<0, 2> = a.div(b);
    ///
    /// assert_eq!(output.get(), 1);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// let a = RangedI128::<-1, 3>::new_const::<2>();
    /// let b = RangedU32::<2, 3>::new_const::<2>();
    /// let output: RangedI128::<-1, 27> = a.pow(b);
    ///
    ...
    /// let a = RangedI128::<1, 3>::new_const::<2>();
    /// let b = RangedU32::<2, 3>::new_const::<2>();
    /// let output: RangedI128::<0, 27> = a.pow(b);
    ///
    /// assert_eq!(output.get(), 4);
    /// ```
    #[must_use = "this returns the result of the operation, \
...
impl<const MIN: i128, const MAX: i128> crate::error::Saturate
    for RangedI128<MIN, MAX>
{
