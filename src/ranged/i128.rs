...
impl<const MIN: i128, const MAX: i128> RangedI128<MIN, MAX> {
    ...
    /// RangedI128::<1, 2>::with_i128(1).unwrap();
    /// RangedI128::<1, 2>::with_i128(2).unwrap();
    /// assert_eq!(RangedI128::<1, 2>::with_i128(0).unwrap_err(), Error::NegOverflow);
    ...
    pub const fn saturating_div(
        ...
        Quotient::Number(match Self::with_i128(self.get().saturating_div(rhs)) {
            Ok(value) => value,
            Err(Error::NegOverflow) => Self::MIN,
            Err(Error::PosOverflow) => Self::MAX,
        })
