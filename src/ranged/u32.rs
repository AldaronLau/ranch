...
impl<const MIN: u32, const MAX: u32> RangedU32<MIN, MAX> {
    ...
    ///
    /// ```rust
    /// # use ranch::{RangedU32, Error};
    /// RangedU32::<1, 2>::with_u32(1).unwrap();
    /// RangedU32::<1, 2>::with_u32(2).unwrap();
    /// assert_eq!(RangedU32::<1, 2>::with_u32(0).unwrap_err(), Error::NegOverflow);
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// RangedU32::<1, 3>::new_const::<1>();
    /// RangedU32::<1, 3>::new_const::<2>();
    /// RangedU32::<1, 3>::new_const::<3>();
    /// ```
    ///
    /// Does not compile:
    ///
    /// ```compile_fail
    /// RangedU32::<1, 3>::new_const::<0>();
    /// ```
    ///
    /// ```compile_fail
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let n = RangedU32::<{ u32::MIN }, { u32::MAX }>::MAX;
    ///
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let n = RangedU32::<0, 255>::new_const::<0b0101000>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<0, 255>::new_const::<0b100_0000>();
    /// let b = RangedU32::<0, 255>::new_const::<0b100_0011>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<1, 100>::new_const::<50>();
    /// let b = RangedU32::<1, 100>::new_const::<5>();
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedU32};
    /// let a = RangedU32::<0, 100>::new_const::<50>();
    /// let b = RangedU32::<0, 100>::new_const::<5>();
    /// let c = RangedU32::<0, 100>::new_const::<75>();
    ///
    ...
    /// let a = RangedU32::<0, 100>::new_const::<50>();
    /// let b = RangedU32::<0, 100>::new_const::<5>();
    /// let c = RangedU32::<0, 100>::new_const::<2>();
    ///
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedU32, Quotient};
    /// let a = RangedU32::<1, 50>::new_const::<50>();
    /// let b = RangedU32::<1, 50>::new_const::<1>();
    ///
    /// assert_eq!(
    ///     a.checked_div(2),
    ///     Some(Quotient::Number(RangedU32::new_const::<25>())),
    /// );
    ...
    /// assert_eq!(
    ///     a.saturating_div(2),
    ///     Quotient::Number(RangedU32::new_const::<25>()),
    /// );
    /// assert_eq!(a.saturating_div(0), Quotient::Nan);
    /// assert_eq!(
    ///     b.saturating_div(2),
    ///     Quotient::Number(RangedU32::new_const::<1>()),
    /// );
    /// ```
    #[must_use = "this returns the result of the operation, \
                  without modifying the original"]
    ...
    /// ```rust
    /// # use ranch::{Error, RangedU32};
    /// let a = RangedU32::<0, 33>::new_const::<0>();
    /// let b = RangedU32::<0, 33>::new_const::<9>();
    /// let c = RangedU32::<0, 33>::new_const::<32>();
    /// let d = RangedU32::<0, 33>::new_const::<33>();
    ///
    ...
    /// ```rust
    /// # use ranch::{Error, RangedU32};
    /// let a = RangedU32::<0, 32>::new_const::<0>();
    /// let b = RangedU32::<0, 32>::new_const::<9>();
    /// let c = RangedU32::<0, 32>::new_const::<32>();
    /// let d = RangedU32::<0, 32>::new_const::<1>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<0, 8>::new_const::<0>();
    /// let b = RangedU32::<0, 8>::new_const::<2>();
    /// let c = RangedU32::<0, 8>::new_const::<4>();
    /// let d = RangedU32::<0, 8>::new_const::<3>();
    /// let e = RangedU32::<0, 8>::new_const::<7>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<1, 3>::new_const::<1>();
    /// let b = RangedU32::<1, 3>::new_const::<2>();
    /// let output: RangedU32::<2, 6> = a.add(b);
    ///
    ...
    /// let a = RangedU32::<1, 3>::new_const::<1>();
    /// let b = RangedU32::<1, 3>::new_const::<2>();
    /// let output: RangedU32::<1, 6> = a.add(b);
    ///
    /// assert_eq!(output.get(), 3);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<2, 5>::new_const::<3>();
    /// let b = RangedU32::<1, 2>::new_const::<1>();
    /// let output: RangedU32::<0, 4> = a.sub(b);
    ///
    ...
    /// let a = RangedU32::<2, 5>::new_const::<3>();
    /// let b = RangedU32::<1, 2>::new_const::<1>();
    /// let output: RangedU32::<0, 3> = a.sub(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// let output: RangedU32::<2, 9> = a.mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    ///
    ...
    /// let a = RangedU32::<1, 3>::new_const::<1>();
    /// let b = RangedU32::<2, 3>::new_const::<2>();
    /// let output: RangedU32::<1, 9> = a.mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<2, 5>::new_const::<3>();
    /// let b = RangedU32::<1, 2>::new_const::<2>();
    /// let output: RangedU32::<1, 2> = a.div(b);
    ///
    ...
    /// let a = RangedU32::<2, 5>::new_const::<3>();
    /// let b = RangedU32::<1, 2>::new_const::<1>();
    /// let output: RangedU32::<0, 2> = a.div(b);
    ///
    /// assert_eq!(output.get(), 1);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// ```rust
    /// # use ranch::RangedU32;
    /// let a = RangedU32::<1, 3>::new_const::<2>();
    /// let b = RangedU32::<2, 3>::new_const::<2>();
    /// let output: RangedU32::<1, 27> = a.pow(b);
    ///
    ...
    /// let a = RangedU32::<1, 3>::new_const::<2>();
    /// let b = RangedU32::<2, 3>::new_const::<2>();
    /// let output: RangedU32::<0, 27> = a.pow(b);
    ///
    /// assert_eq!(output.get(), 4);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        RangedU32(self.get().pow(rhs.get()))
    }
}
