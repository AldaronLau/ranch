...
#[repr(transparent)]
pub struct RangedNonZeroI64<const MIN: i64, const MAX: i64>(pub(crate) i64);
...
impl<const MIN: i64, const MAX: i64> RangedNonZeroI64<MIN, MAX> {
    ...
    ///
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// RangedNonZeroI64::<1, 3>::new::<1>();
    /// RangedNonZeroI64::<1, 3>::new::<2>();
    /// RangedNonZeroI64::<1, 3>::new::<3>();
    /// ```
    ///
    /// Does not compile:
    ///
    /// ```compile_fail
    /// RangedNonZeroI64::<1, 3>::new::<0>();
    /// ```
    ///
    /// ```compile_fail
    /// RangedNonZeroI64::<1, 3>::new::<4>();
    /// ```
    #[must_use]
    ...
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroI64, Error};
    /// RangedNonZeroI64::<1, 2>::with_i64(1).unwrap();
    /// RangedNonZeroI64::<1, 2>::with_i64(2).unwrap();
    /// assert_eq!(RangedNonZeroI64::<1, 2>::with_i64(0).unwrap_err(), Error::NegOverflow);
    /// assert_eq!(RangedNonZeroI64::<1, 2>::with_i64(3).unwrap_err(), Error::PosOverflow);
    /// ```
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// assert_eq!(42, RangedNonZeroI64::<1, 100>::new::<42>().get());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let n = RangedNonZeroI64::<{ i64::MIN }, { i64::MAX }>::MAX;
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let n = RangedNonZeroI64::<-128, 127>::new::<0b0101000>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<-128, 127>::new::<0b100_0000>();
    /// let b = RangedNonZeroI64::<-128, 127>::new::<0b100_0011>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<1, 100>::new::<50>();
    /// let b = RangedNonZeroI64::<1, 100>::new::<5>();
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<-100, 100>::new::<50>();
    /// let b = RangedNonZeroI64::<-100, 100>::new::<5>();
    /// let c = a.saturating_add(b);
    /// let d = RangedNonZeroI64::<-100, 100>::new::<-75>();
    ///
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedNonZeroI64};
    /// let a = RangedNonZeroI64::<-100, 100>::new::<50>();
    /// let b = RangedNonZeroI64::<-100, 100>::new::<5>();
    /// let c = RangedNonZeroI64::<-100, 100>::new::<-75>();
    ///
    ...
    /// let b = RangedNonZeroI64::<-100, 100>::new::<5>();
    /// let c = RangedNonZeroI64::<-100, 100>::new::<-75>();
    /// let d = RangedNonZeroI64::<-100, 100>::new::<2>();
    ///
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedNonZeroI64, Quotient};
    /// let a = RangedNonZeroI64::<-100, 10>::new::<-50>();
    /// let b = RangedNonZeroI64::<-10, 100>::new::<50>();
    ///
    /// assert_eq!(
    ///     a.checked_div(2),
    ///     Ok(Quotient::Number(RangedNonZeroI64::new::<-25>())),
    /// );
    ...
    /// assert_eq!(
    ///     a.saturating_div(2),
    ///     Quotient::Number(RangedNonZeroI64::new::<-25>()),
    /// );
    /// assert_eq!(a.saturating_div(0), Quotient::Nan);
    /// assert_eq!(
    ///     a.saturating_div(-1),
    ///     Quotient::Number(RangedNonZeroI64::new::<10>()),
    /// );
    /// assert_eq!(
    ///     b.saturating_div(-2),
    ///     Quotient::Number(RangedNonZeroI64::new::<-10>()),
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// assert!(!RangedNonZeroI64::<-100, 100>::new::<10>().is_negative());
    /// assert!(RangedNonZeroI64::<-100, 100>::new::<-10>().is_negative());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// assert!(RangedNonZeroI64::<-100, 100>::new::<10>().is_positive());
    /// assert!(!RangedNonZeroI64::<-100, 100>::new::<-10>().is_positive());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<-8, 8>::new::<0>();
    /// let b = RangedNonZeroI64::<-8, 8>::new::<2>();
    /// let c = RangedNonZeroI64::<-8, 8>::new::<4>();
    /// let d = RangedNonZeroI64::<-8, 8>::new::<-1>();
    /// let e = RangedNonZeroI64::<-8, 8>::new::<-7>();
    /// let f = RangedNonZeroI64::<-8, 8>::new::<-3>();
    /// let g = RangedNonZeroI64::<-8, 8>::new::<3>();
    /// let h = RangedNonZeroI64::<-8, 8>::new::<7>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<1, 3>::new::<1>();
    /// let b = RangedNonZeroI64::<-1, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<0, 6> = a.ranged_add(b);
    ///
    ...
    /// let a = RangedNonZeroI64::<1, 3>::new::<1>();
    /// let b = RangedNonZeroI64::<-1, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<1, 6> = a.ranged_add(b);
    ///
    /// assert_eq!(output.get(), 3);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        RangedNonZeroI64(self.get() + rhs.get())
    }
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<2, 5>::new::<3>();
    /// let b = RangedNonZeroI64::<-1, 3>::new::<1>();
    /// let output: RangedNonZeroI64::<-1, 6> = a.ranged_sub(b);
    ///
    ...
    /// let a = RangedNonZeroI64::<2, 5>::new::<3>();
    /// let b = RangedNonZeroI64::<-1, 3>::new::<1>();
    /// let output: RangedNonZeroI64::<0, 6> = a.ranged_sub(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<-2, 3>::new::<1>();
    /// let b = RangedNonZeroI64::<0, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<-6, 9> = a.ranged_mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    ///
    ...
    /// let a = RangedNonZeroI64::<-2, 3>::new::<1>();
    /// let b = RangedNonZeroI64::<0, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<0, 9> = a.ranged_mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<2, 5>::new::<3>();
    /// let b = RangedNonZeroI64::<1, 2>::new::<2>();
    /// let output: RangedNonZeroI64::<1, 2> = a.ranged_div(b);
    ///
    ...
    /// # use ranch::RangedNonZeroI64;
    /// let a = RangedNonZeroI64::<2, 5>::new::<3>();
    /// let b = RangedNonZeroI64::<1, 2>::new::<1>();
    /// let output: RangedNonZeroI64::<0, 2> = a.ranged_div(b);
    ///
    /// assert_eq!(output.get(), 1);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroI64, RangedNonZeroU32};
    /// let a = RangedNonZeroI64::<-1, 3>::new::<2>();
    /// let b = RangedNonZeroU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<-1, 27> = a.ranged_pow(b);
    ///
    ...
    /// let a = RangedNonZeroI64::<1, 3>::new::<2>();
    /// let b = RangedNonZeroU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroI64::<0, 27> = a.ranged_pow(b);
    ///
    /// assert_eq!(output.get(), 4);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        RangedNonZeroI64(self.get().pow(rhs.get()))
    }
}
