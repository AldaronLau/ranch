...
#[repr(transparent)]
pub struct RangedNonZeroU32<const MIN: u32, const MAX: u32>(pub(crate) u32);
...
impl<const MIN: u32, const MAX: u32> RangedNonZeroU32<MIN, MAX> {
    ...
    /// RangedNonZeroU32::<1, 3>::new::<1>();
    /// RangedNonZeroU32::<1, 3>::new::<2>();
    /// RangedNonZeroU32::<1, 3>::new::<3>();
    /// ```
    ///
    /// Does not compile:
    ///
    /// ```compile_fail
    /// RangedNonZeroU32::<1, 3>::new::<0>();
    /// ```
    ///
    /// ```compile_fail
    /// RangedNonZeroU32::<1, 3>::new::<4>();
    /// ```
    #[must_use]
    ...
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroU32, Error};
    /// RangedNonZeroU32::<1, 2>::with_u32(1).unwrap();
    /// RangedNonZeroU32::<1, 2>::with_u32(2).unwrap();
    /// assert_eq!(RangedNonZeroU32::<1, 2>::with_u32(0).unwrap_err(), Error::NegOverflow);
    /// assert_eq!(RangedNonZeroU32::<1, 2>::with_u32(3).unwrap_err(), Error::PosOverflow);
    /// ```
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// assert_eq!(42, RangedNonZeroU32::<1, 100>::new::<42>().get());
    /// ```
    #[must_use]
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let n = RangedNonZeroU32::<{ u32::MIN }, { u32::MAX }>::MAX;
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let n = RangedNonZeroU32::<0, 255>::new::<0b0101000>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let a = RangedNonZeroU32::<0, 255>::new::<0b100_0000>();
    /// let b = RangedNonZeroU32::<0, 255>::new::<0b100_0011>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let a = RangedNonZeroU32::<1, 100>::new::<50>();
    /// let b = RangedNonZeroU32::<1, 100>::new::<5>();
    ...
    pub const fn saturating_add(self, other: impl AsRepr<u32>) -> Self {
        ...
        match Self::with_u32(self.get().saturating_add(other)) {
            Ok(value) => value,
            Err(_) => Self::MAX,
        }
    }
    ...
    /// let a = RangedNonZeroU32::<0, 100>::new::<50>();
    /// let b = RangedNonZeroU32::<0, 100>::new::<5>();
    /// let c = RangedNonZeroU32::<0, 100>::new::<75>();
    ///
    ...
    pub const fn saturating_mul(self, other: impl AsRepr<u32>) -> Self {
        ...
        match Self::with_u32(self.get().saturating_mul(other)) {
            Ok(value) => value,
            Err(_) => Self::MAX,
        }
    }
    ...
    /// let a = RangedNonZeroU32::<0, 100>::new::<50>();
    /// let b = RangedNonZeroU32::<0, 100>::new::<5>();
    /// let c = RangedNonZeroU32::<0, 100>::new::<2>();
    ///
    ...
    pub const fn saturating_pow(self, other: impl AsRepr<u32>) -> Self {
        ...
        match Self::with_u32(self.get().saturating_pow(other)) {
            Ok(value) => value,
            Err(_) => Self::MAX,
        }
    }
    ...
    ///
    /// ```rust
    /// # use ranch::{Error, RangedNonZeroU32, Quotient};
    /// let a = RangedNonZeroU32::<1, 50>::new::<50>();
    /// let b = RangedNonZeroU32::<1, 50>::new::<1>();
    ///
    /// assert_eq!(
    ///     a.checked_div(2),
    ///     Some(Quotient::Number(RangedNonZeroU32::new::<25>())),
    /// );
    ...
    /// assert_eq!(
    ///     a.saturating_div(2),
    ///     Quotient::Number(RangedNonZeroU32::new::<25>()),
    /// );
    /// assert_eq!(a.saturating_div(0), Quotient::Nan);
    /// assert_eq!(
    ///     b.saturating_div(2),
    ///     Quotient::Number(RangedNonZeroU32::new::<1>()),
    /// );
    /// ```
    #[must_use = "this returns the result of the operation, \
                  without modifying the original"]
    pub const fn saturating_div(self, rhs: impl AsRepr<u32>) -> Quotient<Self> {
        ...
        Quotient::Number(match Self::with_u32(self.get().saturating_div(rhs)) {
            Ok(value) => value,
            Err(_) => Self::MIN,
        })
    }
    ...
    pub const fn saturating_sub(self, other: impl AsRepr<u32>) -> Self {
        ...
        match Self::with_u32(self.get().saturating_sub(other)) {
            Ok(value) => value,
            Err(_) => Self::MIN,
        }
    }
    ...
    /// ```rust
    /// # use ranch::{Error, RangedNonZeroU32};
    /// let a = RangedNonZeroU32::<0, 33>::new::<0>();
    /// let b = RangedNonZeroU32::<0, 33>::new::<9>();
    /// let c = RangedNonZeroU32::<0, 33>::new::<32>();
    /// let d = RangedNonZeroU32::<0, 33>::new::<33>();
    ///
    ...
    pub const fn checked_next_power_of_two(self) -> Option<Self> {
        ...
        Some(match Self::with_u32(value) {
            Ok(value) => value,
            Err(_) => return None,
        })
    }
    ...
    /// ```rust
    /// # use ranch::{Error, RangedNonZeroU32};
    /// let a = RangedNonZeroU32::<0, 32>::new::<0>();
    /// let b = RangedNonZeroU32::<0, 32>::new::<9>();
    /// let c = RangedNonZeroU32::<0, 32>::new::<32>();
    /// let d = RangedNonZeroU32::<0, 32>::new::<1>();
    ///
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let a = RangedNonZeroU32::<0, 8>::new::<0>();
    /// let b = RangedNonZeroU32::<0, 8>::new::<2>();
    /// let c = RangedNonZeroU32::<0, 8>::new::<4>();
    /// let d = RangedNonZeroU32::<0, 8>::new::<3>();
    /// let e = RangedNonZeroU32::<0, 8>::new::<7>();
    ///
    ...
    pub const fn midpoint(self, rhs: Self) -> Self {
        let Ok(value) = Self::with_u32(self.get().midpoint(rhs.get())) else {
    ...
    /// let output: RangedNonZeroU32::<2, 6> = a.ranged_add(b);
    ///
    ...
    /// let a = RangedNonZeroU32::<1, 3>::new::<1>();
    /// let b = RangedNonZeroU32::<1, 3>::new::<2>();
    /// let output: RangedNonZeroU32::<1, 6> = a.ranged_add(b);
    ///
    /// assert_eq!(output.get(), 3);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        RangedNonZeroU32(self.get() + rhs.get())
    }
    ...
    /// ```rust
    /// # use ranch::RangedNonZeroU32;
    /// let a = RangedNonZeroU32::<2, 5>::new::<3>();
    /// let b = RangedNonZeroU32::<1, 2>::new::<1>();
    /// let output: RangedNonZeroU32::<0, 4> = a.ranged_sub(b);
    ///
    ...
    /// let a = RangedNonZeroU32::<2, 5>::new::<3>();
    /// let b = RangedNonZeroU32::<1, 2>::new::<1>();
    /// let output: RangedNonZeroU32::<0, 3> = a.ranged_sub(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// let output: RangedNonZeroU32::<2, 9> = a.ranged_mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    ///
    ...
    /// let a = RangedNonZeroU32::<1, 3>::new::<1>();
    /// let b = RangedNonZeroU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroU32::<1, 9> = a.ranged_mul(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// # use ranch::RangedNonZeroU32;
    /// let a = RangedNonZeroU32::<2, 5>::new::<3>();
    /// let b = RangedNonZeroU32::<1, 2>::new::<2>();
    /// let output: RangedNonZeroU32::<1, 2> = a.ranged_div(b);
    ///
    ...
    /// let a = RangedNonZeroU32::<2, 5>::new::<3>();
    /// let b = RangedNonZeroU32::<1, 2>::new::<1>();
    /// let output: RangedNonZeroU32::<0, 2> = a.ranged_div(b);
    ///
    /// assert_eq!(output.get(), 1);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    /// let b = RangedNonZeroU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroU32::<1, 27> = a.ranged_pow(b);
    ///
    ...
    /// let a = RangedNonZeroU32::<1, 3>::new::<2>();
    /// let b = RangedNonZeroU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroU32::<0, 27> = a.ranged_pow(b);
    ///
    /// assert_eq!(output.get(), 4);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        RangedNonZeroU32(self.get().pow(rhs.get()))
    }
}
    ...
    for RangedNonZeroU32<MIN, MAX>
{
    ...
    fn from_str(src: &str) -> ParsingResult<Self> {
        let parsed = src.parse::<u32>()?;

        Self::with_u32(parsed).map_err(From::from)
    }
}

impl<const MIN: u32, const MAX: u32> crate::error::Clamp
    for RangedNonZeroU32<MIN, MAX>
{
