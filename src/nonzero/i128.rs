...
impl<const MIN: i128, const MAX: i128> RangedNonZeroI128<MIN, MAX> {
    ...
    pub const fn checked_add(
        ...
        match as_repr::as_repr::<RangedI128<MIN, MAX>>(self).checked_add(other)
        {
    ...
    pub const fn checked_mul(
        ...
        match as_repr::as_repr::<RangedI128<MIN, MAX>>(self).checked_mul(other)
        {
            Ok(value) => Ok(value.to_ranged_nonzero()),
            Err(e) => Err(e),
        }
    ...
    pub const fn checked_pow(self, other: impl AsRepr<u32>) -> Result<Self> {
        match as_repr::as_repr::<RangedI128<MIN, MAX>>(self).checked_pow(other)
        {
    ...
    pub const fn checked_div(
        ...
        let value = match as_repr::as_repr::<RangedI128<MIN, MAX>>(self)
            .checked_div(rhs)
        {
            Ok(value) => value,
            Err(e) => return Err(e),
    ...
    pub const fn checked_sub(
        ...
        match as_repr::as_repr::<RangedI128<MIN, MAX>>(self).checked_sub(other)
        {
            Ok(value) => Ok(value.to_ranged_nonzero()),
            Err(e) => Err(e),
        }
    ...
    /// let output: RangedNonZeroI128::<-6, 9> = a.mul_ranged(b);
    ///
    ...
    /// let a = RangedNonZeroI128::<-2, 3>::new::<1>();
    /// let b = RangedNonZeroI128::<-1, 3>::new::<2>();
    /// let output: RangedNonZeroI128::<0, 9> = a.mul_ranged(b);
    ///
    /// assert_eq!(output.get(), 2);
    /// ```
    #[must_use = "this returns the result of the operation, \
    ...
    >(
        ...
        as_repr::as_repr::<RangedI128<MIN, MAX>>(self)
    ...
    /// let output: RangedNonZeroI128::<-1, 27> = a.pow_ranged(b);
    ///
    ...
    /// let a = RangedNonZeroI128::<1, 3>::new::<2>();
    /// let b = RangedU32::<2, 3>::new::<2>();
    /// let output: RangedNonZeroI128::<0, 27> = a.pow_ranged(b);
    ///
    /// assert_eq!(output.get(), 4);
    /// ```
    #[must_use = "this returns the result of the operation, \
