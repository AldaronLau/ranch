...
use crate::{
    Error, ParsingError, ParsingResult, Quotient, RangedI32, RangedU32, Result,
};
...
impl<const MIN: i32, const MAX: i32> RangedNonZeroI32<MIN, MAX> {
    ...
    /// Convert from [`RangedI32`].
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroI32, RangedI32};
    /// assert_eq!(
    ///     RangedNonZeroI32::<1, 100>::with_ranged(RangedI32::new::<42>()).unwrap(),
    ///     RangedNonZeroI32::<1, 100>::new::<42>(),
    /// );
    /// assert!(
    ///     RangedNonZeroI32::<0, 100>::with_ranged(RangedI32::new::<0>())
    ///         .is_none(),
    /// );
    /// ```
    pub const fn with_ranged(ranged: RangedI32<MIN, MAX>) -> Option<Self> {
    ...
    >(
        ...
        RangedNonZeroI32::with_ranged(self.to_ranged().ranged_pow(rhs)).unwrap()
    }
}
    ...
    for RangedNonZeroI32<MIN, MAX>
{
    ...
    fn from_str(src: &str) -> ParsingResult<Self> {
        let parsed = src.parse::<NonZero<i32>>()?;

        Self::with_i32(parsed.get())
            .transpose()
            .unwrap()
            .map_err(From::from)
    }
