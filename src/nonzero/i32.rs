...
impl<const MIN: i32, const MAX: i32> RangedNonZeroI32<MIN, MAX> {
    ...
    pub const fn checked_add(
        ...
        match as_repr::as_repr::<RangedI32<MIN, MAX>>(self).checked_add(other) {
    ...
    pub const fn checked_mul(
        ...
        match as_repr::as_repr::<RangedI32<MIN, MAX>>(self).checked_mul(other) {
            Ok(value) => Ok(value.to_ranged_nonzero()),
            Err(e) => Err(e),
        }
    }
    ...
    pub const fn checked_pow(self, other: impl AsRepr<u32>) -> Result<Self> {
        match as_repr::as_repr::<RangedI32<MIN, MAX>>(self).checked_pow(other) {
    ...
    pub const fn checked_div(
        ...
        let value = match as_repr::as_repr::<RangedI32<MIN, MAX>>(self)
            .checked_div(rhs)
        {
            Ok(value) => value,
            Err(e) => return Err(e),
    ...
    pub const fn checked_sub(
        ...
        match as_repr::as_repr::<RangedI32<MIN, MAX>>(self).checked_sub(other) {
            Ok(value) => Ok(value.to_ranged_nonzero()),
            Err(e) => Err(e),
        }
    }
