...
impl<const MIN: u64, const MAX: u64> RangedNonZeroU64<MIN, MAX> {
    ...
    /// Convert from [`RangedU64`].
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroU64, RangedU64};
    /// assert_eq!(
    ///     RangedNonZeroU64::<1, 100>::with_ranged(RangedU64::new::<42>()).unwrap(),
    ///     RangedNonZeroU64::<1, 100>::new::<42>(),
    /// );
    /// assert!(
    ///     RangedNonZeroU64::<0, 100>::with_ranged(RangedU64::new::<0>())
    ///         .is_none(),
    /// );
    /// ```
    pub const fn with_ranged(ranged: RangedU64<MIN, MAX>) -> Option<Self> {
    ...
    >(
        ...
        RangedNonZeroU64::with_ranged(self.to_ranged().ranged_pow(rhs)).unwrap()
    }
