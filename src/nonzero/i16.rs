...
use crate::{
    Error, ParsingError, ParsingResult, Quotient, RangedI16, RangedU32, Result,
};

/// [`i16`] not to equal zero with a specified minimum and maximum value
#[derive(Copy, Clone, Eq, PartialEq, Ord, PartialOrd, Hash)]
#[repr(transparent)]
pub struct RangedNonZeroI16<const MIN: i16, const MAX: i16>(
    pub(crate) NonZero<i16>,
);
...
impl<const MIN: i16, const MAX: i16> RangedNonZeroI16<MIN, MAX> {
    ...
    /// RangedNonZeroI16::<-1, 1>::with_i16(-1).unwrap().unwrap();
    /// RangedNonZeroI16::<-1, 1>::with_i16(1).unwrap().unwrap();
    /// assert_eq!(RangedNonZeroI16::<-1, 1>::with_i16(0), Ok(None));
    /// assert_eq!(RangedNonZeroI16::<-1, 1>::with_i16(-2).unwrap_err(), Error::NegOverflow);
    /// assert_eq!(RangedNonZeroI16::<-1, 1>::with_i16(2).unwrap_err(), Error::PosOverflow);
    /// ```
    pub const fn with_i16(value: impl AsRepr<i16>) -> Result<Option<Self>> {
    ...
    /// # use ranch::RangedNonZeroI16;
    /// assert_eq!(
    ///     RangedNonZeroI16::<1, 100>::with_nonzero(NonZero::new(42).unwrap()).unwrap(),
    ///     RangedNonZeroI16::<1, 100>::new::<42>(),
    /// );
    /// ```
    pub const fn with_nonzero(
        nonzero: impl AsRepr<NonZero<i16>>,
    ) -> Result<Self> {
        const { Self::assert_range() };
    ...
    /// Convert from [`RangedI16`].
    ///
    /// ```rust
    /// # use ranch::{RangedNonZeroI16, RangedI16};
    /// assert_eq!(
    ///     RangedNonZeroI16::<1, 100>::with_ranged(RangedI16::new::<42>()).unwrap(),
    ///     RangedNonZeroI16::<1, 100>::new::<42>(),
    /// );
    /// assert!(
    ///     RangedNonZeroI16::<0, 100>::with_ranged(RangedI16::new::<0>())
    ///         .is_none(),
    /// );
    /// ```
    pub const fn with_ranged(ranged: RangedI16<MIN, MAX>) -> Option<Self> {
    ...
    ///     RangedNonZeroI16::<1, 100>::new::<42>().to_nonzero(),
    /// );
    /// ```
    #[must_use]
    pub const fn to_nonzero(self) -> NonZero<i16> {
        self.0
    }
    ...
    /// assert_eq!(
    ///     a.checked_div(2),
    ///     Ok(Some(Quotient::Number(RangedNonZeroI16::new::<-25>()))),
    /// );
    ...
    for RangedNonZeroI16<MIN, MAX>
{
    ...
    fn from_str(src: &str) -> ParsingResult<Self> {
        let parsed = src.parse::<NonZero<i16>>()?;

        Self::with_i16(parsed.get())
            .transpose()
            .unwrap()
            .map_err(From::from)
    }
