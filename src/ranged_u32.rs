...
impl<const MIN: u32, const MAX: u32> RangedU32<MIN, MAX> {
    ...
    /// RangedU32::<1, 2>::new(1).unwrap();
    /// RangedU32::<1, 2>::new(2).unwrap();
    /// assert_eq!(RangedU32::<1, 2>::new(0).unwrap_err(), Error::NegOverflow);
    /// assert_eq!(RangedU32::<1, 2>::new(3).unwrap_err(), Error::PosOverflow);
    /// ```
    pub const fn new(value: impl AsRepr<u32>) -> Result<Self> {
